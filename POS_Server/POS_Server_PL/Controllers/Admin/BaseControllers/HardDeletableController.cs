...
public class HardDeletableController<TModel, TService> : BaseController<TModel, TService>
    where TModel : BaseModel where TService : IHardDeletableService<TModel>
{
    ...
    [HttpGet("GetAll")]
    public virtual async Task<ActionResult<OperationResult<List<TModel>>>> GetAllAsync()
    {
        OperationResult<List<TModel>> result = await _hardService.GetAllAsync();
    ...
    {
        OperationResult<List<TModel>> result = await _hardService.GetAllPagedAsync(pageQty, pageNum);
        if (!result.Succeeded || result.Data == null) return HandleInternalFailure(result);
        foreach (TModel model in result.Data) _service.RemoveNavigationProps(model);
        return Ok(result);
    }
    ...
    {
        OperationResult<List<TModel>> result = await _hardService.FilterByAsync(predicate);
        if (!result.Succeeded || result.Data == null) return HandleInternalFailure(result);
        foreach (TModel model in result.Data) _service.RemoveNavigationProps(model);
        return Ok(result);
    }
        ...
        Expression<Func<TModel, bool>> predicate, int pageQty, int pageNum)
    {
        OperationResult<List<TModel>> result =
            await _hardService.FilterByPagedAsync(predicate, pageQty, pageNum);
